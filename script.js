...
function initMap() {
    ...
    google.maps.event.addListener(
        map,
        "bounds_changed",() => {
            fetch(`http://api.openweathermap.org/data/2.5/box/city?appid=a087c537ef6c31ec6ad3237a859addad&bbox=${map.getMapBbox()}`)
            .then(function(response) {
                response.json().then(function(data) {
                  if (data.list) {
                      if (!downloadedCities.includes(data.list[0].name)) {
                        fetch(`http://api.openweathermap.org/data/2.5/weather?q=${data.list[0].name}&appid=a087c537ef6c31ec6ad3237a859addad`)
                        .then(function(res) {
